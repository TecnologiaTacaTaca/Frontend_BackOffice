...
@ui.page('/fraude/auditoria')
@plantilla
async def auditoria():
    """Página de Auditoría - Próximamente"""
    ...
    with ui.column().classes('w-full h-full items-center justify-center'):
        with ui.card().classes('p-8 shadow-lg border-2 border-red-500'):
            ...
            with ui.row().classes('items-center gap-2'):
                ui.icon('search', size='1.5rem').classes('text-red-400')
                ui.label('Auditoría').classes('text-2xl font-semibold text-red-600')
                ui.icon('fact_check', size='1.5rem').classes('text-red-400')
